...
import { PersonSearch } from '../../src/components/PersonOldSearch';
...
// const mockUseApi = useApi as jest.MockedFunction<
// 	<List>(
// 		apiToCall: (...args: any) => Promise<List>,
...
describe('The PersonSearch component', () => {
	...
	// 	expect(mockUseApi).toHaveBeenCalledWith(searchPersonsByNameSearch, {});
	// });
	...
	// 			createUseApiResponseWithPersons(threePersonObjects)
	// 		);
...
function createUseApiResponseWithPersons(persons: Person[]): {
	result: {
		hasData: boolean;
		isError: boolean;
		data?: List;
	...
	const list = new List(persons, 1, toNumber, toNumber * 2);

	return {
		result: { hasData: true, isError: false, data: list },
		isLoading: false,
